...
from keyboards.default.buttons import get_before_url, rek_types, send_button, admin_button
...
@dp.message(F.text == '📌 Bekor qilish',TextSend.url, IsBotAdmin())
async def cancel(message: types.Message, state: FSMContext):
    await message.answer('📲  Reklama yubiorish turini tanlang:', reply_markup=rek_types())
    await state.clear()


@dp.message(F.text == '📌 Bekor qilish', TextSend.check, IsBotAdmin())
async def cancel(message: types.Message, state: FSMContext):
    await message.answer('📲  Reklama yubiorish turini tanlang:', reply_markup=rek_types())
    await state.clear()
